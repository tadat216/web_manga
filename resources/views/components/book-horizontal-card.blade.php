        ...
        <div class="flex-1 p-4 flex flex-col justify-between">
            <div>
                <h2 class="text-lg font-semibold flex items-center gap-2 flex-wrap">
                    ...
                    @if(Auth::check() && isset($book->users) && $book->users->first())
                    <p class="text-sm text-green-600 flex items-center gap-1 mb-2">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                        </svg>
                        Đã xem đến: Chương {{ $book->lastReadChapters->first()->chapter_number ?? '?' }}
                    </p>
                    @endif
                </h2>
                ...
                <p class="text-gray-600 flex items-center gap-1">
                    ...
                    Thể loại: {{ $book->genres->pluck('title')->join(', ') }}
                </p>
                <p class="text-gray-600 flex items-center gap-1">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
